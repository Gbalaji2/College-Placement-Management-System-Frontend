...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${BASE_URL}/api/v1/student/login`, formData);
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-indigo-400 via-sky-300 to-emerald-500">
        <form className="bg-white/30 p-8 rounded-lg shadow backdrop-blur-md w-1/3 max-md:w-3/4 flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="relative">
            <input type={isEyeOpen ? "text" : "password"} name="password" placeholder="Password" value={formData.password} onChange={handleChange} className="form-control pr-10" />
          ...
          <p className="text-center mt-2">
            Don't have an account? <span className="text-blue-500 cursor-pointer" onClick={() => navigate('../student/signup')}>Sign Up</span>
          </p>
        </form>
      </div>
    </>
